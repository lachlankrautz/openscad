...
module side_notch_v3(diameter, width, fraction = 1, bleed = 0, trim = false) {
...
module notch_support_v3(tile_size, wall_inset_length) {
...
module lid_side_notches_v3(box_size, wall_inset_length, bleed=0, trim=false) {
  notch_padding = 0;
  ...
  translate([
    ...
    side_notch_v3(bump_diameter, $wall_thickness, bleed=bleed, trim=trim);
  }
...
module corner_cube_v3(size, facing=1, rounding=$rounding) {
  _size = [
    size[0] + rounding,
    size[1] + rounding,
    size[2],
...
module lid_notch_supports_v3(tile_size, box_size, wall_inset_length) {
  tile_stack_size_2d = tile_stack_size(tile_size);
  corner_grip_size = [
...
module corner_grip_cutout_v3(box_size, wall_inset_length) {
  corner_grip_size = [
...
module tile_tray_v3(
  ...
  difference() {
    ...
    // Snap and grip lid
    if (with_lid) {
      corner_grip_cutout_v3(box_size, wall_inset_length);
      translate([0, 0, box_size[2] - bump_diameter - bump_drop_height - corner_grip_height]) {
        lid_side_notches_v3(box_size, wall_inset_length, bleed=$bleed);
      }
    }
  }
}

module tile_tray_lid_v3(tile_size, matrix, matrix_counts, wall_inset_length) {
  box_size = [
  ...
  union() {
    ...
    translate([0, 0, $wall_thickness + bump_drop_height + corner_grip_height]) {
      lid_side_notches_v3(box_size, wall_inset_length + x_tolerance / 2, trim=true);
    }
    lid_notch_supports_v3(tile_size, box_size, wall_inset_length);
  }
}
